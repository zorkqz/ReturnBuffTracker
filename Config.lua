			...
			general = {
				...
				args = {
					...
					hideNotInRaid = {
						...
						type = "toggle",
						order = 3,
						get = function(self) return ReturnBuffTracker.db.profile.hideNotInRaid end,
...
function ReturnBuffTracker:ChatCommand(input)
	if not input or input:trim() == "" then
		InterfaceOptionsFrame_OpenToCategory(ReturnBuffTracker.optFrames.ReturnBuffTracker)
		 -- Workaround: https://www.wowinterface.com/forums/showthread.php?t=54599
		InterfaceOptionsFrame_OpenToCategory(ReturnBuffTracker.optFrames.ReturnBuffTracker)
	else
		LibStub("AceConfigCmd-3.0").HandleCommand(ReturnBuffTracker, "ReturnBuffTracker", "ReturnBuffTracker", input)
	end
end

function ReturnBuffTracker:SetupOptions()
	ReturnBuffTracker.optFrames = {}
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable("ReturnBuffTracker", getOptions)
	ReturnBuffTracker.optFrames.ReturnBuffTracker = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("ReturnBuffTracker", "Return Buff Tracker", nil, "general")
